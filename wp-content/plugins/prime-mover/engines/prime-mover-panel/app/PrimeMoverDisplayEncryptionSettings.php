...
class PrimeMoverDisplayEncryptionSettings
{
    ...
    private function outputMarkup()
    {
        ...
        if (!$this->getPrimeMover()->getSystemFunctions()->isConfigFileWritable()) {
            $readonly = 'readonly';
            $disabled = 'disabled';
            $title = esc_attr__('You can only change the encryption key by editing PRIME_MOVER_DB_ENCRYPTION_KEY constant in wp-config.php.', 'prime-mover');
        }
        ?>
